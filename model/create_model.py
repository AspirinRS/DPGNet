...
def get_model(backbone_name='mobilenetv2', fpn_channels=128, deform_groups=4,
    ...
    detector = Detector(backbone_name, fpn_channels, num_heads, num_points, kernel_layers, dropout_rate, init_type)
...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        self.detector = get_model(backbone_name=opt.backbone, fpn_channels=opt.fpn_channels,
    ...
    def forward(self, x1, x2, label):
        pred, pred_d1, pred_d2, pred_d3, pred_d4 = self.detector(x1, x2)
        label = label.long()
        focal = self.focal(pred, label)
        dice = self.dice(pred, label)
        d1_loss = self.focal(pred_d1, label) * 0.5 + self.dice(pred_d1, label)
        d2_loss = self.focal(pred_d2, label) * 0.5 + self.dice(pred_d2, label)
        d3_loss = self.focal(pred_d3, label) * 0.5 + self.dice(pred_d3, label)
        d4_loss = self.focal(pred_d4, label) * 0.5 + self.dice(pred_d4, label)

        return focal, dice, d1_loss, d2_loss, d3_loss, d4_loss

    def inference(self, x1, x2):
        with torch.no_grad():
            pred, *_ = self.detector(x1, x2)
        return pred
